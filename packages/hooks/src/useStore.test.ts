...
class FakePollingStore extends PollingStore {
  private syncImpl: () => Promise<void>

  constructor(syncImpl: () => Promise<void>, opts: { initialState: ProjectionState }) {
    super({ initialState: opts.initialState })
...
describe('useStore', () => {
  ...
  describe('syncState', () => {
    ...
    it('picks up syncState from a PollingStore on notification', async () => {
      const store = new FakePollingStore(async () => {}, { initialState })
      const { result } = renderHook(() => useStore(store, initialState))
    ...
    it('reflects error health after a failed sync', async () => {
      const store = new FakePollingStore(async () => { throw new Error('network') }, { initialState })
      const { result } = renderHook(() => useStore(store, initialState))
      await act(async () => { await store.sync() })
