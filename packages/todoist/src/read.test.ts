...
// ── buildEvents: single-item Todoist→palimpsest task mapping ──────────────────
// Each case is one Todoist item in, and the resulting task.created event (or its absence) out —
// a pure, stateless mapping, so it enumerates cleanly as a table rather than one it() per field.

interface TaskMappingCase {
  name: string
  item: Partial<SyncItem> & { id: string }
  projects?: SyncProject[]
  // Fields expected on the task.created event (checked with toMatchObject — only the keys
  // mentioned are checked), or `false` for "no task.created event at all".
  expectTask?: Record<string, unknown> | false
  // Keys expected ABSENT from the task.created event.
  expectAbsent?: string[]
  // Whether a task.completed event should also be emitted for this item.
  expectCompleted?: boolean
}

const TASK_MAPPING_CASES: TaskMappingCase[] = [
  { name: 'emits task.created for a work one-offs task with sphereId',
    item: { id: 't1' },
    expectTask: { type: 'task.created', taskId: 't1', sphereId: WORK_SPHERE_ID },
    expectAbsent: ['projectId'] },

  { name: 'emits task.created with projectId for a task in a regular project',
    item: { id: 't1', project_id: 'proj1' },
    projects: [makeProject({ id: 'proj1', parent_id: TODOIST_WORK_PROJECT_ID })],
    expectTask: { projectId: 'proj1' },
    expectAbsent: ['sphereId'] },

  { name: 'emits task.created + task.completed for a completed non-recurring task',
    item: { id: 't1', checked: true, completed_at: '2026-06-01T10:00:00.000Z' },
    expectTask: {},
    expectCompleted: true },

  { name: 'emits task.created (without dueDateExpression) + task.completed for a recurring task completed forever',
    item: { id: 't1', checked: true, completed_at: '2026-06-01T10:00:00.000Z',
      due: { date: '2026-07-07', is_recurring: true, string: 'every monday' } },
    expectTask: {},
    expectAbsent: ['dueDateExpression'],
    expectCompleted: true },

  { name: 'emits no events for a deleted task',
    item: { id: 't1', is_deleted: true },
    expectTask: false },

  { name: 'does not set waitingFor for a task with only the waiting label',
    item: { id: 't1', labels: ['waiting'] },
    expectAbsent: ['waitingFor'] },

  { name: 'sets waitingFor review only when waiting is paired with nonagenda',
    item: { id: 't1', labels: ['waiting', 'nonagenda'] },
    expectTask: { waitingFor: { kind: 'review' } } },

  { name: 'sets waitingFor agenda when waiting is paired with an agenda label',
    item: { id: 't1', labels: ['waiting', 'jim'] },
    expectTask: { waitingFor: { kind: 'agenda', agendaId: 'agenda-jim' } } },

  // ── Inbox sphere resolution ──────────────────────────────────────────────
  { name: 'a plain Inbox task with no personal label defaults to Work sphere',
    item: { id: 't1', project_id: TODOIST_INBOX_ID },
    expectTask: { sphereId: WORK_SPHERE_ID } },

  { name: 'an Inbox task carrying the personal label resolves to Personal sphere',
    item: { id: 't1', project_id: TODOIST_INBOX_ID, labels: ['personal'] },
    expectTask: { sphereId: PERSONAL_SPHERE_ID } },
  ...
  { name: 'a dated Inbox task marked unsphered is skipped entirely, not defaulted to Work',
    item: {
  ...
  { name: 'imports a task living in a work agenda project as project-less, with that agendaId and sphere',
    item: { id: 't1', project_id: AGENDA_ID_TO_AGENDA_PROJECT_ID['agenda-jim']! },
    expectTask: { sphereId: WORK_SPHERE_ID, agendaId: 'agenda-jim' },
    expectAbsent: ['projectId'] },

  { name: 'imports a task living in a personal agenda project with the personal sphere',
    item: { id: 't1', project_id: AGENDA_ID_TO_AGENDA_PROJECT_ID['agenda-han']! },
    expectTask: { sphereId: PERSONAL_SPHERE_ID, agendaId: 'agenda-han' },
    expectAbsent: ['projectId'] },

  { name: 'an explicit agenda label on a task in a regular project still resolves agendaId normally',
    item: { id: 't1', project_id: 'proj1', labels: ['jim'] },
    projects: [makeProject({ id: 'proj1', parent_id: TODOIST_WORK_PROJECT_ID })],
    expectTask: { projectId: 'proj1', agendaId: 'agenda-jim' } },

  { name: 'an explicit agenda label takes priority over a conflicting agenda-project inference',
    item: { id: 't1', project_id: AGENDA_ID_TO_AGENDA_PROJECT_ID['agenda-jim']!, labels: ['marcia'] },
    expectTask: { agendaId: 'agenda-marcia' } },

  { name: 'derives waitingFor kind agenda from the project-inferred agendaId when only the waiting label is present',
    item: { id: 't1', project_id: AGENDA_ID_TO_AGENDA_PROJECT_ID['agenda-jim']!, labels: ['waiting'] },
    expectTask: { waitingFor: { kind: 'agenda', agendaId: 'agenda-jim' } } },
]

describe('buildEvents — single-item task mapping', () => {
  it.each(TASK_MAPPING_CASES)('$name', ({ item, projects, expectTask, expectAbsent, expectCompleted }) => {
    const events = buildEvents([...CONTAINERS, ...(projects ?? [])], [makeItem(item)])
    const created = events.find(e => e.type === 'task.created' && e.taskId === item.id)

    if (expectTask === false) {
      expect(created).toBeUndefined()
    } else {
      expect(created).toBeDefined()
      if (expectTask !== undefined) expect(created).toMatchObject(expectTask)
    }
    for (const key of expectAbsent ?? []) expect(created).not.toHaveProperty(key)
    if (expectCompleted !== undefined) {
      expect(events.some(e => e.type === 'task.completed' && e.taskId === item.id)).toBe(expectCompleted)
    }
  })
})

// ── buildEvents: everything else ──────────────────────────────────────────────
// Multi-item inputs, project-event assertions, or projected-state checks — these don't reduce to
// a single item → single task.created row the way the table above does.
...
describe('buildEvents — tasks (multi-item inputs)', () => {
...
describe('buildEvents — agenda-specific project events', () => {
...
// ── buildDeltaEvents ──────────────────────────────────────────────────────────
// Delta events depend on a `current: ProjectionState` argument representing prior state, and
// several cases assert on sequences of multiple events or on project-level agenda transitions —
// none of this reduces to a flat single-item input→output row, so these stay as plain it()s.
