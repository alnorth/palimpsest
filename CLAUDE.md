...
`search.ts`'s `searchAll(state, query, opts?)` builds a fresh `minisearch` index from scratch on every call rather than caching one — like the rest of this package (and `core/query.ts` underneath it), it stays entirely stateless and recomputes from `ProjectionState` each time; at personal-task-manager scale (hundreds to low thousands of items, not a search-engine corpus) the rebuild cost is negligible against never going stale. Search options are `{ prefix: true, fuzzy: 0.2 }` — `prefix` matches a partial word at the end of the query (e.g. `"groc"` matches `"groceries"`, enabling find-as-you-type in `packages/hooks`), `fuzzy` tolerates small typos.
...
`useSearch(query, filter?)` is the one read hook whose first argument is a plain string rather than a filter object, since it's built for find-as-you-type: pass the raw text-input value straight through on every keystroke, no debouncing needed — `@alnorth/palimpsest-query`'s `search` command (see packages/query section above) rebuilds its MiniSearch index fresh per call, which is cheap at this scale, and `internal/useRunQuery.ts`'s existing `JSON.stringify(command)`-keyed memoization already skips recomputation when the query string hasn't actually changed between renders. A blank/whitespace-only `query` passes `command: undefined` to `useRunQuery` (same as any other read hook with nothing to query yet) and returns an empty-but-valid `ListResult` (`{ data: [], isLoading: false, error: undefined, total: 0, truncated: false }`) — a fresh search box starts empty, not loading. `filter` is `{ sphere?, includeArchived?, limit? }`, mirroring the `search` command's own optional fields.
