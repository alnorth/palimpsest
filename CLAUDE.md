...
Read hooks: `useSpheres`, `useAgendas`, `useContexts`, `useProjects`, `useTasks`, `useTask`, `useDashboard`, `useProcessing`, `useWaiting`, `usePickList`, `useSearch`, `useSyncStatus`, `useCurrentSphere`. Every read hook returns a **plain value** — `TaskJson`/`ProjectJson`/etc. directly for single-value/aggregate results, or a `Paginated<T>` (`{ items, total, truncated }`) for lists — never a `{data, isLoading, error}`-style envelope. Instead, a read hook suspends (via React 19's `use()`) while the Provider's store is still connecting, and throws — both a query-level error (e.g. `runQuery` failing to resolve a sphere/agenda/context name) and the store's own connection failure — for the consuming app's own `<ErrorBoundary>` to catch. There is no `isLoading`/`error` field to check inline; a consuming app wraps its tree in `<Suspense fallback={...}>` and an `<ErrorBoundary>` the same way it would for any other Suspense-based data source. Only mutation hooks (below) keep an imperative `{ mutate, isPending, error }` shape — Suspense is a render-phase mechanism and has no equivalent for writes.
