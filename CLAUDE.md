...
Read hooks: `useSpheres`, `useAgendas`, `useContexts`, `useProjects`, `useProject`, `useTasks`, `useTask`, `useDashboard`, `useProcessing`, `useWaiting`, `usePickList`, `useSearch`, `useAgenda`, `useSyncStatus`, `useCurrentSphere`. `useProject(id: string)` is the project-side sibling of `useTask`, backed by `packages/query`'s `project` command kind (see that package's section above) — a raw id, not a name (same reasoning as the write hooks), returning `QueryResult<ProjectJson>`. `useAgenda(agenda: string, sphere?: string)` is the hooks-side face of `packages/query`'s `agenda_view` command (see that package's section above) — a single required `agenda` name/id argument, no `usePalimpsestContext` sphere-fallback (unlike `useDashboard`/`usePickList`), returning `QueryResult<AgendaViewData>` where `AgendaViewData` is `{ agenda: AgendaJson, waitingTasks: TaskJson[], activeTasks: TaskJson[], projects: ProjectJson[] }`. Every read hook returns a consistent envelope — `QueryResult<T>` (`{ data, isLoading, error }`) for single-value/aggregate results, `ListResult<T>` (adds `total`/`truncated`) for paginated lists — and returns plain denormalized `TaskJson`/`ProjectJson`/etc., never pre-formatted strings.
