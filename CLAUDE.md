...
A local MCP server over the stdio transport, for use by MCP clients (e.g. Claude Desktop, Claude Code). Exposes twelve read-only tools mirroring `@alnorth/palimpsest-query`'s `ParsedCommand` kinds one-for-one (`tasks`, `task`, `projects`, `spheres`, `agendas`, `contexts`, `dashboard`, `processing`, `waiting`, `pick_list`, `search`, `agenda_view`) plus five write tools — `complete_task`, `set_due_date`, `set_starred`, `delete_task`, `set_project_agenda` — a small but growing set alongside the read ones.

```
src/
  index.ts        — entry point: builds the store, connects McpServer over StdioServerTransport
  server.ts       — createMcpServer(store): registers the 17 tools with zod input schemas
  tools.ts        — pure per-tool handlers: read handlers map args → ParsedCommand and call runQuery;
                    ...
                    handleDeleteTask) map args → a core command → store.appendEvents, via the shared
                    runTaskToolMutation scaffolding; the one project write handler
                    (handleSetProjectAgenda) uses the project-flavored runProjectToolMutation
                    sibling instead (see below)
...
All five write handlers take a plain `id` (task or project) — no name resolution, unlike the collection filters, since a wrong fuzzy match on a *write* is a much worse failure mode than on a read; this applies equally to `set_project_agenda`'s `agendaId` argument (a raw id, not a name — an MCP client resolves it via the `agendas` tool first, the same way it already must for the target `id` itself). The four task write handlers share `runTaskToolMutation(store, taskId, buildEvents)`: sync → look up the task via core's `getTask` (throws `Task not found: ...` if missing) → `buildEvents(task)` to get the event(s) to append (`completeTask` → `task.completed`, or `task.recurred` if the task carries a `dueDateExpression`; `updateTask(task, { dueDate })` → `task.updated`; `updateTask(task, { isStarred })` → `task.updated`; `deleteTask` → `task.deleted`) → `appendEvents` → a second `store.sync()` to flush the pending event through immediately rather than leaving it for `PollingStore`'s debounced sync → re-read state → `runQuery(..., { kind: 'task', id })`. `TodoistStore.sync()` swallows network failures internally (sets `syncState.health` to `'error'` instead of rejecting), so a failed flush looks identical to a successful one from the handler's `await` alone — `runTaskToolMutation` checks `store.syncState` afterwards and reports `synced: false` plus a `warning` string when the flush didn't actually confirm, rather than silently claiming confirmation that didn't happen. The write itself already succeeded at that point (`appendEvents` resolved), so this is reported as an unsynced success (`ok: true, synced: false`), not a tool error.

`set_project_agenda` (`{ id, agendaId?: string | null, selfOnly?: boolean }`, `null` clearing `agendaId` via `CLEAR`) is the one write tool operating on a `Project` instead of a `Task` — its `runProjectToolMutation` sibling is structurally identical to `runTaskToolMutation`, differing only in entity type: sync → `getProject` (throws `Project not found: ...`) → `updateProject(project, { agendaId?, isSelfOnly? })` → `appendEvents` (surfacing core's own mutual-exclusivity throw as `isError: true` if the call sets both a non-null `agendaId` and `selfOnly: true` — `{ agendaId: null, selfOnly: true }` is not contradictory and is allowed through, same reasoning as core's check; and surfacing core's same-sphere validation error as `isError: true` if the target agenda is in a different sphere than the project) → flush sync → re-read via `runQuery(..., { kind: 'project', id })` (see packages/query section above) → same `synced`/`warning` reporting.

`set_due_date`'s `dueDate` input is `string | null` — a string (`"today"` or an ISO `YYYY-MM-DD`, resolved by a local `resolveDueDate` helper mirroring `@alnorth/palimpsest-query`'s `resolveDateArg`) to set it, or `null` to clear it via core's `CLEAR` sentinel. There is no dedicated core command for this — it goes through the generic `updateTask(task, { dueDate })` patch, same as `postponeTask` does internally. `set_starred`'s `starred` input is a plain boolean, going through the same generic `updateTask(task, { isStarred })` patch — no resolution needed. `delete_task`, `set_due_date`, and `set_starred` all fail (via `updateTask`/`deleteTask`'s own guards) if the task isn't in a valid state for the change — e.g. `set_due_date`/`set_starred` on a completed/deleted task, or `delete_task` on an already-deleted one — surfaced the same way as `complete_task`'s "already completed" guard. `Task.isStarred` here is palimpsest's own concept, mapped by `packages/todoist` onto Todoist's native `priority: 4` field (see that package's section above) — it is unrelated to the dashboard app's own "starred" bookmark list (a hidden-task JSON blob covering both Todoist tasks and Trello cards), which this tool does not attempt to reproduce.
