...
A local MCP server over the stdio transport, for use by MCP clients (e.g. Claude Desktop, Claude Code). Exposes twelve read-only tools mirroring `@alnorth/palimpsest-query`'s `ParsedCommand` kinds one-for-one (`tasks`, `task`, `projects`, `spheres`, `agendas`, `contexts`, `dashboard`, `processing`, `waiting`, `pick_list`, `search`, `agenda_view`) plus six write tools — `complete_task`, `set_due_date`, `delete_task`, `set_task_sphere`, `set_project_agenda`, `set_project_sphere` — a small but growing set alongside the read ones.

```
src/
  index.ts        — entry point: builds the store, connects McpServer over StdioServerTransport
  server.ts       — createMcpServer(store): registers the 18 tools with zod input schemas
  tools.ts        — pure per-tool handlers: read handlers map args → ParsedCommand and call runQuery;
                    task write handlers (handleCompleteTask/handleSetDueDate/handleDeleteTask/
                    handleSetTaskSphere) map args → a core command → store.appendEvents, via the
                    shared runTaskToolMutation scaffolding; the project write handlers
                    (handleSetProjectAgenda/handleSetProjectSphere) use the project-flavored
                    runProjectToolMutation sibling instead (see below)
