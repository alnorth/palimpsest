...
Past the PoC stage: work on a feature branch and open a pull request for review, rather than
committing directly on `main`. Branch names follow the pattern already used by this repo's own
in-flight branches (a short kebab-case slug describing the change, optionally prefixed with the
author's name, e.g. `alnorth/add-all-write-tasks-needed-for-daily-dashboard-v`).
...
**`commands.ts`** — Pure functions `(ProjectionState, input) → PalimpsestEvent[]`. They validate inputs and produce events but never write to disk. `completeTask` branches on whether the task has a `dueDateExpression`: recurring tasks get `task.recurred` (due date advances, task stays open); non-recurring tasks get `task.completed` (task closed). `updateTask` enforces the domain model's "belongs to a project OR carries a direct sphereId, never both" invariant (see Domain model below) whenever a patch actually touches `projectId`/`sphereId`: it computes the *effective* value of each (the patch's own value if present, else the task's current one, via `resolvePatched`) and throws if both would end up defined (`'A task cannot have both a projectId and a direct sphereId'`) or if neither would (`'Task must belong to a project or have a direct sphereId'`, the same message `createTask` uses) — mirroring how `updateProject`'s `agendaId`/`isSelfOnly` mutual-exclusivity check re-derives effective values rather than diffing the raw patch. An unrelated patch (e.g. `{ title }`) skips the check entirely and never re-rejects a task that already carries a legacy invalid combination the read path tolerated when folding it into state. This is what lets `set_task_sphere` (packages/mcp) / `useSetTaskSphere` (packages/hooks) safely expose `updateTask(task, { sphereId })` as a public write: a project-linked task's sphere is derived (`getTaskSphereId`), so setting `sphereId` directly on one would silently have no effect on the query side — the guard turns that footgun into an explicit error instead. Moving a task onto a project therefore requires clearing its direct sphereId in the same patch (`updateTask(task, { projectId, sphereId: CLEAR })`), same as the existing "assign task to project" call sites already do.
...
`ParsedCommand` kinds: `tasks`, `task`, `spheres`, `agendas`, `contexts` (the original CLI-era surface, filters matching `core/query.ts`'s `TaskFilter` field-for-field, plus `dueOn`/`dueBefore`/`limit`), `projects` (same CLI-era surface, called out below for its `includeNextTasks` option), four aggregate views ported from the old interactive TUI's view model, with no equivalent anywhere else in the codebase, `search`, and `agenda_view` (a newer aggregate view, ported from the dashboard app's own Agendas.jsx rather than the TUI — see below):

- **`project`** — `{ kind: 'project', id: string }`. Fetches a single project by id (throws `No project with id "..."` if missing), the project-side sibling of `task`. Returns `{ project: ProjectJson }` with stats computed via `computeProjectStats`/`statsFor`, same as each entry in the `projects` list below — unlike `task` id lookups don't go through `resolve.ts` (a raw id, not a name, same reasoning as the MCP/hooks write handlers taking raw ids).
...
- **`agenda_view`** — `{ kind: 'agenda_view', agenda: string, sphere?: string }` (`agenda` name/id resolved via `resolveAgenda`, `sphere` an optional disambiguator like the other commands). Ports the dashboard app's `Agendas.jsx` "Agenda" view: for one agenda, returns `{ agenda: AgendaJson, waitingTasks: TaskJson[], activeTasks: TaskJson[], projects: ProjectJson[] }`. Backed by `views.ts`'s `agendaView(state, agendaId, today)`, which reuses `core/query.ts`'s existing `isActionable` filter (`status === 'open' && (no real project || isNext)`) as the task-eligibility rule — this already matches dashboard's own two OR'd criteria once generalized: a task with no real project (which, per the `packages/todoist` section below, now covers both an explicitly labelled free-floating task and one inferred from living in an agenda-specific project) always counts; a task that does belong to a real project additionally needs `isNext`. Eligible tasks are further filtered to undated or due today/earlier (mirrors dashboard's `taskIsDue`), then split into `waitingTasks`/`activeTasks` purely by `waitingFor` presence — the same split, orthogonal to which half of the eligibility rule matched. `projects` is a separate list — non-archived projects with `agendaId` set to this agenda (the "shared projects" blob link from the `projects` command above) — populated independently of the task lists, exactly mirroring dashboard's separate "Shared projects" section rather than folding those projects' tasks into the eligibility check. Exposed as `packages/mcp`'s `agenda_view` tool and `packages/hooks`' `useAgenda` hook.
...
`runTaskToolMutation` call it unconditionally (mcp only supports the Todoist store); `packages/hooks`'
...
A local MCP server over the stdio transport, for use by MCP clients (e.g. Claude Desktop, Claude Code). Exposes twelve read-only tools mirroring `@alnorth/palimpsest-query`'s `ParsedCommand` kinds one-for-one (`tasks`, `task`, `projects`, `spheres`, `agendas`, `contexts`, `dashboard`, `processing`, `waiting`, `pick_list`, `search`, `agenda_view`) plus seven write tools — `complete_task`, `set_due_date`, `set_starred`, `delete_task`, `set_task_sphere`, `set_project_agenda`, `set_project_sphere` — a small but growing set alongside the read ones.

```
src/
  index.ts        — entry point: builds the store, connects McpServer over StdioServerTransport
  server.ts       — createMcpServer(store): registers the 19 tools with zod input schemas
  tools.ts        — pure per-tool handlers: read handlers map args → ParsedCommand and call runQuery;
                    task write handlers (handleCompleteTask/handleSetDueDate/handleSetStarred/
                    handleDeleteTask/handleSetTaskSphere) map args → a core command →
                    store.appendEvents, via the shared runTaskToolMutation scaffolding; the project
                    write handlers (handleSetProjectAgenda/handleSetProjectSphere) use the
                    project-flavored runProjectToolMutation sibling instead (see below)
...
All seven write handlers take a plain `id` (task or project) — no name resolution, unlike the collection filters, since a wrong fuzzy match on a *write* is a much worse failure mode than on a read; this applies equally to `set_project_agenda`'s `agendaId` argument and `set_task_sphere`/`set_project_sphere`'s `sphereId` argument (raw ids, not names — an MCP client resolves them via the `agendas`/`spheres` tools first, the same way it already must for the target `id` itself). The five task write handlers share `runTaskToolMutation(store, taskId, buildEvents)`: sync → look up the task via core's `getTask` (throws `Task not found: ...` if missing) → `buildEvents(task)` to get the event(s) to append (`completeTask` → `task.completed`, or `task.recurred` if the task carries a `dueDateExpression`; `updateTask(task, { dueDate })` → `task.updated`; `updateTask(task, { isStarred })` → `task.updated`; `updateTask(task, { sphereId })` → `task.updated`; `deleteTask` → `task.deleted`) → `appendEvents` → a second `store.sync()` to flush the pending event through immediately rather than leaving it for `PollingStore`'s debounced sync → re-read state → `runQuery(..., { kind: 'task', id })`. `TodoistStore.sync()` swallows network failures internally (sets `syncState.health` to `'error'` instead of rejecting), so a failed flush looks identical to a successful one from the handler's `await` alone — `runTaskToolMutation` checks `store.syncState` afterwards and reports `synced: false` plus a `warning` string when the flush didn't actually confirm, rather than silently claiming confirmation that didn't happen. The write itself already succeeded at that point (`appendEvents` resolved), so this is reported as an unsynced success (`ok: true, synced: false`), not a tool error.

`set_project_agenda` (`{ id, agendaId?: string | null, selfOnly?: boolean }`, `null` clearing `agendaId` via `CLEAR`) and `set_project_sphere` (`{ id, sphereId }`) are the two write tools operating on a `Project` instead of a `Task` — their `runProjectToolMutation` sibling is structurally identical to `runTaskToolMutation`, differing only in entity type: sync → `getProject` (throws `Project not found: ...`) → `updateProject(project, { agendaId?, isSelfOnly? })` / `updateProject(project, { sphereId })` → `appendEvents` (surfacing core's own mutual-exclusivity throw as `isError: true` if `set_project_agenda` sets both a non-null `agendaId` and `selfOnly: true` — `{ agendaId: null, selfOnly: true }` is not contradictory and is allowed through, same reasoning as core's check; and surfacing core's same-sphere validation error as `isError: true` if `set_project_agenda`/`set_project_sphere` would leave the project linked to an agenda in a different sphere) → flush sync → re-read via `runQuery(..., { kind: 'project', id })` (see packages/query section above) → same `synced`/`warning` reporting.

`set_due_date`'s `dueDate` input is `string | null` — a string (`"today"` or an ISO `YYYY-MM-DD`, resolved by a local `resolveDueDate` helper mirroring `@alnorth/palimpsest-query`'s `resolveDateArg`) to set it, or `null` to clear it via core's `CLEAR` sentinel. There is no dedicated core command for this — it goes through the generic `updateTask(task, { dueDate })` patch, same as `postponeTask` does internally. `set_starred`'s `starred` input is a plain boolean, going through the same generic `updateTask(task, { isStarred })` patch — no resolution needed; `Task.isStarred` here is palimpsest's own concept, mapped by `packages/todoist` onto Todoist's native `priority: 4` field (see that package's section above) — it is unrelated to the dashboard app's own "starred" bookmark list (a hidden-task JSON blob covering both Todoist tasks and Trello cards), which this tool does not attempt to reproduce. `set_task_sphere`'s `sphereId` is a required, non-nullable string — a project-less task's sphere is mandatory (unlike its due date), so there's no clearing form. It fails (via core's `updateTask` guard, see packages/core section above) if the task belongs to a project, since a project-linked task's effective sphere is derived from the project (`getTaskSphereId`) and can't be overridden directly — move the task to a different project instead. `delete_task`, `set_due_date`, `set_starred`, and `set_task_sphere` all fail (via `updateTask`/`deleteTask`'s own guards) if the task isn't in a valid state for the change — e.g. `set_due_date`/`set_starred`/`set_task_sphere` on a completed/deleted task, or `delete_task` on an already-deleted one — surfaced the same way as `complete_task`'s "already completed" guard.
...
src/
  PalimpsestProvider.tsx        — Context + Provider: fuses the connect phase (store.init() → getState())
                                  ...
                                  store, stateResource, projState, isConnecting, connectionError, syncState,
                                  refresh, currentSphereId/setCurrentSphere, today
  useStore.ts                   — lower-level subscribe/poll hook over an already-known ProjectionState;
                                  never suspends by construction (see below)
  ...
  internal/useRunQuery.ts       — shared memoized runQuery(projState, command) wrapper every read hook uses;
                                  the Suspense resource mechanism lives here (see below)
...
Read hooks: `useSpheres`, `useAgendas`, `useContexts`, `useProjects`, `useProject`, `useTasks`, `useTask`, `useDashboard`, `useProcessing`, `useWaiting`, `usePickList`, `useSearch`, `useAgenda`, `useSyncStatus`, `useCurrentSphere`. Every read hook returns a **plain value** — `TaskJson`/`ProjectJson`/etc. directly for single-value/aggregate results, or a `Paginated<T>` (`{ items, total, truncated }`) for lists — never a `{data, isLoading, error}`-style envelope. Instead, a read hook suspends (via React 19's `use()`) while the Provider's store is still connecting, and throws — both a query-level error (e.g. `runQuery` failing to resolve a sphere/agenda/context name) and the store's own connection failure — for the consuming app's own `<ErrorBoundary>` to catch. There is no `isLoading`/`error` field to check inline; a consuming app wraps its tree in `<Suspense fallback={...}>` and an `<ErrorBoundary>` the same way it would for any other Suspense-based data source. Only mutation hooks (below) keep an imperative `{ mutate, isPending, error }` shape — Suspense is a render-phase mechanism and has no equivalent for writes. `useProject(id: string)` is the project-side sibling of `useTask`, backed by `packages/query`'s `project` command kind (see that package's section above) — a raw id, not a name (same reasoning as the write hooks), returning `ProjectJson` directly. `useAgenda(agenda: string, sphere?: string)` is the hooks-side face of `packages/query`'s `agenda_view` command (see that package's section above) — a single required `agenda` name/id argument, no `usePalimpsestContext` sphere-fallback (unlike `useDashboard`/`usePickList`), returning `AgendaViewData` (`{ agenda: AgendaJson, waitingTasks: TaskJson[], activeTasks: TaskJson[], projects: ProjectJson[] }`) directly, same plain-value/throws-on-error convention as every other read hook.

**The Suspense resource mechanism** (`PalimpsestProvider.tsx` + `internal/useRunQuery.ts`): the Provider exposes both `stateResource: Promise<ProjectionState>` (the store's connect promise — `store.init().then(() => store.getState())`, created once per `store` identity) and `projState: ProjectionState | undefined` (a plain, always-in-sync mirror of `stateResource`'s last resolved value, updated on every `store.subscribe()` notification, not just the first). `useRunQuery` calls `use(stateResource)` **only while `projState` is still undefined** — once a value exists, it reads the mirror directly and never calls `use()` again for the rest of that store's lifetime. This isn't just an optimization: publishing a fresh `Promise.resolve(state)` to `use()` on every live update was tried and rejected, because even an already-fulfilled promise costs a real (if brief) suspend/resume cycle the first time `use()` sees that particular promise object — React can't know it's settled without a microtask round trip, so the Suspense fallback would flash on every poll refresh. Reading the plain mirror instead means only the store's *initial* connect ever suspends, matching the pre-Suspense design's one-shot `isLoading` semantics exactly. `use()` is called conditionally in `useRunQuery` (allowed for `use`, unlike other hooks) so that a read hook with no command yet (e.g. `useDashboard`/`usePickList` before a sphere is resolved) never forces suspension on a store that hasn't connected. `useMutation`'s `mutate` callback (an event handler, not a render) always reads the plain `projState` mirror, since it can't suspend — see below. `useStore.ts` is a separate, lower-level primitive that takes an already-known `initialState` and never has an undefined/connecting state at all, so it has nothing to suspend on and is untouched by any of this.

`useSearch(query, filter?)` is the one read hook whose first argument is a plain string rather than a filter object, since it's built for find-as-you-type: pass the raw text-input value straight through on every keystroke, no debouncing needed. This is cheap specifically because of how `@alnorth/palimpsest-query`'s `search` command is cached (see `search.ts`'s `WeakMap` in the packages/query section above): `PalimpsestProvider`'s `projState` keeps the same object reference across every re-render until the store actually notifies of a change, so typing a query only ever re-runs a cheap `.search()` against an index built once at the start of that session, not a full MiniSearch rebuild per keystroke. `internal/useRunQuery.ts`'s existing `JSON.stringify(command)`-keyed memoization additionally skips recomputation entirely when the query string hasn't actually changed between renders. A blank/whitespace-only `query` passes `command: undefined` to `useRunQuery` (same as any other read hook with nothing to query yet, and the same reason it must not suspend) and returns an empty-but-valid `Paginated` (`{ items: [], total: 0, truncated: false }`) — a fresh search box starts empty, not suspended. `filter` is `{ sphere?, includeArchived?, limit? }`, mirroring the `search` command's own optional fields.

Write hooks: `useCompleteTask()`, `useSetDueDate()`, `useSetStarred()`, `useDeleteTask()`, `useSetTaskSphere()`, `useSetProjectAgenda()`, `useSetProjectSphere()` (more — `useCreateTask()`, etc. — are a planned later phase). Every write hook returns `MutationResult<TArgs, TResult>` (`{ mutate, isPending, error }`) from the shared `internal/useMutation.ts` primitive. `useCompleteTask()` and `useDeleteTask()` take no arguments and return `{ mutate: (taskId: string) => Promise<void>, isPending, error }` — the id varies per call rather than per hook instance, so one hook instance can complete/delete whichever row was just clicked in a list. `useSetDueDate()` returns `{ mutate: (args: SetDueDateArgs) => Promise<void>, isPending, error }` where `SetDueDateArgs` is `{ taskId: string; dueDate: string | null }` (`null` clears the due date via core's `CLEAR` sentinel — there's no dedicated core command for this, it's the same generic `updateTask(task, { dueDate })` patch the MCP `set_due_date` tool uses). `useSetStarred()` is the same shape for the star toggle: `{ mutate: (args: SetStarredArgs) => Promise<void>, isPending, error }`, `SetStarredArgs` = `{ taskId: string; starred: boolean }`, going through the generic `updateTask(task, { isStarred })` patch the MCP `set_starred` tool also uses — this is palimpsest's own `isStarred` concept (mapped to Todoist priority 4 by `packages/todoist`, see that package's section above), not the dashboard app's separate blob-based "starred" bookmark list. `useSetTaskSphere()` returns `{ mutate: (args: SetTaskSphereArgs) => Promise<void>, isPending, error }`, `SetTaskSphereArgs` = `{ taskId: string; sphereId: string }` (no clearing form — a project-less task's sphere is mandatory — same generic `updateTask(task, { sphereId })` patch the MCP `set_task_sphere` tool uses); its `mutate` rejects with core's own "cannot have both a projectId and a direct sphereId" error if the task belongs to a project (see packages/core section above), the same way `requireTask`'s "not found" throw surfaces. `useSetProjectAgenda()` is the project-side equivalent: `{ mutate: (args: SetProjectAgendaArgs) => Promise<void>, isPending, error }`, `SetProjectAgendaArgs` = `{ projectId: string; agendaId?: string | null; selfOnly?: boolean }` (`agendaId: null` clears via `CLEAR`, same `updateProject(project, { agendaId?, isSelfOnly? })` patch the MCP `set_project_agenda` tool uses; `agendaId` is a raw id, no name resolution, for the same reason the MCP tool takes a raw id). The `agendaId`+`selfOnly: true` mutual-exclusivity guard lives only in core's `updateProject` (not duplicated at this layer) — its throw surfaces via `useMutation`'s `error`/rejected-`mutate`-promise, same as `requireProject`'s "not found" throw; `{ agendaId: null, selfOnly: true }` is not contradictory and is allowed through. `useSetProjectSphere()` mirrors `useSetTaskSphere()` for projects: `{ mutate: (args: SetProjectSphereArgs) => Promise<void>, isPending, error }`, `SetProjectSphereArgs` = `{ projectId: string; sphereId: string }`, same generic `updateProject(project, { sphereId })` patch the MCP `set_project_sphere` tool uses; core's same-sphere validation (a linked agenda in a different sphere) surfaces the same way. Every `mutate` looks the task/project up fresh from the *current* `projState` (not a value captured at render time) via the shared `internal/requireTask.ts`/`internal/requireProject.ts` helpers (`getTask`/`getProject` plus the "not found" guard every write hook needs), then calls the matching core command (`completeTask`/`updateTask`/`deleteTask`/`updateProject`) and `store.appendEvents(...)`. No hand-rolled optimistic local state is needed: `PollingStore`-based stores already fold pending (unsynced) events into every `getState()` projection, so the Provider's existing `subscribe`→`getState()`→re-render loop reflects the write as soon as `appendEvents` resolves, well before the debounced network sync completes.

`useMutation`'s `mutate` is wrapped in `useCallback([store, projState, fn])`, so each write hook's `fn` must be a stable reference (module-level, not an inline closure defined inside the hook body) or the memoization is defeated and `mutate` gets a new identity every render regardless of whether `store`/`projState` actually changed. `useCompleteTask`, `useSetDueDate`, `useSetStarred`, `useDeleteTask`, `useSetTaskSphere`, `useSetProjectAgenda`, and `useSetProjectSphere` all follow this by defining their mutation logic as a top-level `runCompleteTask`/`runSetDueDate`/`runSetStarred`/`runDeleteTask`/`runSetTaskSphere`/`runSetProjectAgenda`/`runSetProjectSphere` function and passing that reference to `useMutation`, rather than an inline arrow function — later write hooks should do the same.

Filter param shapes mirror `@alnorth/palimpsest-query`'s `ParsedCommand` fields exactly (same sphere/project/agenda/context/status/etc. vocabulary as the MCP tools), with one exception: **sphere-scoping for the four aggregate hooks is split by hook family**. `useTasks`/`useProjects`/`useAgendas`/`useContexts` stay fully parameterized — an omitted `sphere` never falls back to context state. `useWaiting` mirrors its MCP tool (`sphere` optional, unscoped when omitted). `useDashboard`/`usePickList` mirror their MCP tools' required-`sphere` constraint, but satisfy it at the hook layer by falling back to the Context's `currentSphereId` when their own argument is omitted — if neither resolves, they return an empty-but-valid result (`{ items: [], total: 0, truncated: false }` for `useDashboard`, `[]` for `usePickList`) without ever calling `use()`, never an error. `useProcessing` takes no sphere argument at all, matching its MCP tool's always-global scope.
